...
class TimeTower(nn.Module):
    ...
    def encode(self, timestamps):
        ...
        timestamps = [format(max(0.0, t), '0>6.1f') for t in timestamps]
...
if __name__ == "__main__":
    ...
    time_encoder = TimeTower(tokenizer, hidden_dim=10)
    print(torch.cat([
        time_encoder.encode([12.3, 45.6]),
        time_encoder.encode([12.3]),
        time_encoder.encode([]),
    ], dim=0).shape)
