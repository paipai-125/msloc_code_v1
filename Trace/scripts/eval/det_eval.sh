...
# =============================================================================
# det_eval.sh - run inference of the `det` model.
# - Reads the proposal JSON.
# - For each segment, predicts real/fake.
...
TEST_ANNO_FILE=${TEST_ANNO_FILE:-"$MSLOC_ASSETS/data/Tasle-CoT-10K/annos/test_all_1209_0119_short.json"}
...
OUTPUT_DIR="${MSLOC_ASSETS}/Trace/inference_results/det_${DATASET}_${SPLIT}"
mkdir -p "${OUTPUT_DIR}"
...
echo "=========================================="
echo "det inference (single-machine ${NUM_GPUS}-GPU data parallel)"
echo "=========================================="
...
for i in $(seq 0 $(($NUM_GPUS - 1))); do
  ...
  if [ "$i" -eq 0 ]; then
    CUDA_VISIBLE_DEVICES=$i python -u ${DIR}/trace/eval/evaluate_det.py \
